import {
    ...
    AnimationMixer,
    Skin
} from "../ExportModelUtils.js";
import {Camera} from '../ExportScene.js'
import {Transform} from '../ExportPhysics.js'
...
export class GLTFLoader {
    ...
    async load(gltf) {
        this.gltfUrl = new URL(gltf, window.location);
    ...
    loadMaterial(nameOrIndex) {
        ...
        if (gltfSpec.splatTexture1) {
            options.splatTexture1 = this.loadTexture(gltfSpec.splatTexture1.index);
        }
        if (gltfSpec.splatTexture2) {
            options.splatTexture2 = this.loadTexture(gltfSpec.splatTexture2.index);
        }
        if (gltfSpec.multipleTextures) {
            options.multipleTextures = [];
            for (const key in gltfSpec.multipleTextures) {
                const index = gltfSpec.multipleTextures[key].index;
                options.multipleTextures.push(this.loadTexture(index));
            }
        }
    ...
    loadAccessor(nameOrIndex) {
        ...
        const max = gltfSpec.max ?? null;
        const min = gltfSpec.min ?? null;
    ...
    loadSkin(nameOrIndex) {
        ...
        const joints = gltfSpec.joints.map(jointInd => this.loadNode(jointInd))
        const inverseBindMatrices = this.loadAccessor(gltfSpec.inverseBindMatrices)
        const skin = new Skin({ joints, inverseBindMatrices});
    ...
    loadAnimation(nameOrIndex) {
        ...
        const channels = gltfSpec.channels.map(channelSpec => {
            ...
            const targetNode = this.loadNode(target.node);
            const sampler = this.loadAnimationSampler(channelSpec.sampler);

            return new AnimationChannel({targetNode, target, sampler});
        })
        const animation = new Animation({ channels });
        this.cache.set(gltfSpec, animation);
        return animation;
    }

    loadAnimationSampler(nameOrIndex) {
        const gltfSpec = this.findByNameOrIndex(this.gltf.samplers, nameOrIndex);
        ...
        const inputAccessor = this.loadAccessor(gltfSpec.input);
        const outputAccessor = this.loadAccessor(gltfSpec.output);
    ...
    createMeshFromPrimitive(spec) {
        ...
        return new Mesh({ accessors, vertices, indices });
    }
    ...
    loadNode(nameOrIndex) {
        ...
        if (gltfSpec.skin !== undefined)
            node.addComponent(this.loadSkin(gltfSpec.skin));

        if (gltfSpec.animations !== undefined && gltfSpec.animations.length > 0) {
            const animations = gltfSpec.animations.map(animationInd => this.loadAnimation(animationInd))
            node.addComponent(new AnimationMixer({ animations }))
        }
